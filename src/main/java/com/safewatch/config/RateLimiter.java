...
public class RateLimiter extends OncePerRequestFilter {
    ...
    private Bucket newBucket(String path) {
        ...
        } else if (path.equals("/api/v1/auth/update/password")) {
            limit = Bandwidth.simple(3, Duration.ofMinutes(1));
        } else if (path.equals("/api/v1/auth/update/details")) {
            limit = Bandwidth.simple(3, Duration.ofMinutes(1));
        } else if (path.equals("/api/incident/report")) {
            limit = Bandwidth.simple(10, Duration.ofMinutes(1));
        } else if (path.equals("/api/v1/community/create")) {
            limit = Bandwidth.simple(10, Duration.ofMinutes(1));
        } else {
    ...
    private boolean shouldLimit(String method, String path) {
        return ("POST".equals(method) && (
                path.equals("/api/v1/auth/login") ||
                        ...
                        path.equals("/api/v1/auth/update/password") ||
                        path.equals("/api/v1/auth/update/details") ||
                        path.equals("/api/v1/auth/incident/report") ||
                        path.equals("/api/v1/community/create")
        ));
    }
}
